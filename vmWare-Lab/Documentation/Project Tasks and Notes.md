...
- [x] Now that you're getting IPs from your DHCP server, configure DHCP clustering. Loadbalancing or failover is your choice. Now test it.
...
- [x] Build out another two servers and join it to the domain as member servers.
- [x] Install DFS and File server roles/features on both servers.
- [x] Create a file share on bother servers with the same folder name. Create files on both servers. Make sure they are different. (i.e server1 will have "TextDoc01", server 2 will have "TextDoc02" in their shares).
- [x] Create a DFS name space. Add those shares to the name space.
- [x] On a domain joined work station, navigate to the DFS namespace you created. You should be able to see both files.
- [x] Create a DFS Replication group. Make it so that you have two way replication. You should now see both files on both servers. Make a change on one server and see if it replicates to another server. Does it work? Great. (you can shut down the file servers for now if you want or use them for the next step)

## Create WDS Server

- [x] Create another server, join it to the domain, install Windows Deployment Service (WDS) and Windows Server Update Service (WSUS). You can choose to use the file servers you've already created instead of building out another VM. You only need one file server.
- [x] Configure WDS so that you can PXE boot to it on the network. Make any required changes to routing and DHCP if need be.
- [x] Upload an image to WDS for PXE deployment. Use WAIK and sysprep if you need to. (I haven't done this in the long time so you might not need sysprep anymore with WAIK, look it up)
- [x] Create a new desktop VM but do not install an OS on it. Instead tell it to perform a PXE boot when you turn it on, have it install the OS from here.
- [x] Configure WSUS so that you will only download Security updates for the desktop and server OS's ( highly recommend that you do not download any updates if you have access to the internet from this server)
- [x] Bonus points, install WSUS on another server and create a downstream server
- [x] Create some groups in WSUS. Servers and workstations will do nicely.
- [x] Create a new group policy that points workstations into the WSUS workstation group, points to WSUS for updates, and stop workstations from automatically downloading updates.
- [x] Read up on approving and pushing updates since the current assumption is that there are no updates to be pushed in this enclosed test network since there is no internet access to download them. I believe there is a way to manually add updates to WSUS but I'm a bit foggy on that. Research it.
- [x] Do the same for servers.

## PowerShell

- [x] Create a new AD user via powershell.
- [x] Create a new AD group via powershell.
- [x] Print a list of all domain users and computers in powershell, names only
- [x] Use powershell to pull a list of users who have new york as their office. If no users have new york listed as their - office, use powershell to set that attribute and then pull users who have new york as their office.
- [x] Remove a user account from AD using powershell
- [x] Add a user to a group using powershell
